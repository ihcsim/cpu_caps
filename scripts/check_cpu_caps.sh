...
#
# the container is started with the virt-handler image defined by the
...
function run_debugger() {
  local virt_handlers=($(kubectl -n "${KUBEVIRT_NAMESPACE}" get po \
  ...
  for virt_handler in "${virt_handlers[@]}"; do
    ...
    # collect the virsh capabilities .xml files generated by the custom virt-handler.
    # the debugger references the in-container /var/lib/kubevirt-node-labeller folder, not the one on the host.
    ...
    # collect the virsh capabilities .xml files from the hostpath /var/lib/kubevirt-node-labeller
